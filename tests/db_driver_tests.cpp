...
TEST(driver_test, db_tabel_create)
{
    ...
    try {
        ...
        db_driver.connect(cfg);
        ASSERT_EQ(true, db_driver.is_connected());
        std::string create = "CREATE TABLE IF NOT EXISTS TEST(ID INT PRIMARY KEY NOT NULL);";
...
TEST(driver_test, db_tabel_insert)
{
    bck::sql::config cfg {"test_example.db", "test_bck.txt"};
    try {
        bck::sql::driver db_driver;
        ...
        std::string insert("INSERT INTO TEST VALUES(" + std::to_string(0) + ");");
        auto in_code = db_driver.execute(insert);
        ASSERT_EQ(0, code);
